...
var certificate =  fs.readFileSync('./keys/b_bank_server.crt', 'utf-8');
...
app.post("/login", (req,res)=>{
    ...
    var sql = "SELECT * FROM b_bank WHERE ID = ? AND PW = ?";
    connection.query(
        ...
        if (error) throw error;
            ...
            if (results.length == 0) {
                ...
                if(dbID==hash_id && dbPW==hash_passwd){
                    ...
                    var sql2 = "UPDATE b_bank SET sessionKey = ? WHERE ID=?";
...
app.post("/logout",function(req,res){
    ...
    sql = "SELECT * FROM b_bank WHERE sessionKey = ?";
    connection.query(
        sql, [hash_session_key], function(error, results){
            if(error)   throw error;
                ...
                if(sessionKey = dbSessionKey){
                    ...
                    sql2 = "UPDATE b_bank SET sessionKey = ? WHERE CI = ?"
                    connection.query(
